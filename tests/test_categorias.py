...
def test_crear_categoria():
    with TestClient(app) as client:
        ...
        response = client.post("/categorias/crear_categoria", json={"nombre": "Nueva Categoria"})
...
def test_editar_categoria_inexistente():
    with TestClient(app) as client:
        ...
        response = client.post("/categorias/editar_categoria", json={"categoria_id": 9999, "nuevo_nombre": "Categoria Editada"})
...
def test_editar_categoria_existente():
    with TestClient(app) as client:
        ...
        response = client.post("/categorias/editar_categoria", json={"categoria_id": 25, "nuevo_nombre": "Categoria Editada"})
        #estado 200 significa que se editó la categoría
        assert response.status_code == 200
        #verificamos que la respuesta contenga un mensaje
...
def test_eliminar_categoria_inexistente():
    with TestClient(app) as client:
        ...
        response = client.post("/categorias/eliminar_categoria", json={"id": 9999})
        assert response.status_code == 404
        assert response.json().get("message") == "Categoría no encontrada"
...
def test_eliminar_categoria_existente():
    with TestClient(app) as client:
        ...
        response = client.post("/categorias/eliminar_categoria", json={"id": 1})
        assert response.status_code == 200
